...
Write-Host "4. Khởi động Camera Simulator (Đồng bộ video web)..." -ForegroundColor Yellow
$simulator = Start-Process -FilePath $Python -ArgumentList "scripts\camera_simulator.py" -PassThru -NoNewWindow
...
Write-Host "  .venv\Scripts\python.exe scripts\run_ai_continuous.py"
Write-Host ""
Write-Host "Ctrl+C trong cửa sổ này không dừng server (Start-Process tách rời)." -ForegroundColor DarkGray
Write-Host "Dừng thủ công: Stop-Process -Id $($backend.Id),$($frontend.Id),$($simulator.Id) -Force" -ForegroundColor DarkGray
