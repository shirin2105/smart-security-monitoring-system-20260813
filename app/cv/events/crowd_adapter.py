...
class CrowdLifecycleAdapter:
    ...
    def evaluate(self, tracks: list[Any], frame_data: Any) -> list[EventSignal]:
        ...
        for zone in self.zones:
            ...
            tracker = self._states.setdefault(
                zone_id, CrowdZoneStateTracker(zone_id, self.threshold, self.hold_seconds, self.release_threshold)
            )
            ...
            if current == CrowdState.CROWD_ACTIVE:
                ...
                signals.append(self._signal(zone_id, False, timestamp, now_s, self._last_facts[zone_id]))
        return signals

    def _signal(self, zone_id: str, active: bool, timestamp: str, now_s: float, facts: dict[str, Any]) -> EventSignal:
        return EventSignal(
            self.camera_id,
            "CROWD_THRESHOLD",
            zone_id,
