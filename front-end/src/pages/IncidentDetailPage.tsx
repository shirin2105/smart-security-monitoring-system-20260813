...
export function IncidentDetailPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6">
      <Link
        to="/incidents"
        className="mb-4 inline-flex items-center gap-1.5 text-xs text-gray-400 transition-colors hover:text-blue-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
      >
      ...
      <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
        ...
        <section className="space-y-4 rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <h2 className="font-mono text-xs font-semibold uppercase text-gray-400">
                Mô tả sự cố
              </h2>
          ...
          {event.recommendedSeverity &&
            event.recommendedSeverity !== event.effectiveSeverity && (
              <p className="flex items-start gap-2 rounded-lg border border-gray-800 bg-gray-950/60 p-3 text-[11px] leading-relaxed text-gray-400">
                <ShieldQuestion className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet-400" aria-hidden />
                <span>
                  AI đề xuất mức{' '}
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
            <h2 className="mb-3 font-mono text-xs font-semibold uppercase text-gray-400">
              Hành động của bạn
            </h2>
                ...
                {[...event.actions]
                  ...
                  .map((action) => (
                    <li key={action.id} className="relative">
                      <span
                        className="absolute -left-[27px] top-1.5 h-3 w-3 rounded-full border-2 border-gray-900 bg-blue-500"
                        aria-hidden
                      />
                      <div className="rounded-xl border border-gray-800/80 bg-gray-950/70 p-3">
