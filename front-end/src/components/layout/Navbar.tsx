...
export function Navbar({ streamStatus, onTriggerSimulation }: NavbarProps) {
  ...
  return (
    <header className="glass-panel sticky top-0 z-40 border-b border-gray-800 px-4 py-3 md:px-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        {/* Thương hiệu */}
        <div className="flex items-center gap-3">
          <div className="rounded-lg border border-blue-500/40 bg-blue-600/20 p-2 text-blue-400">
          ...
          <div>
            <h1 className="flex items-center gap-2 text-base font-bold tracking-wide text-white">
              ...
              {isMockMode && (
                <span className="flex items-center gap-1 rounded border border-violet-500/40 bg-violet-500/20 px-2 py-0.5 font-mono text-[10px] text-violet-300">
            ...
            {visibleNav.map(({ to, label, icon: Icon }) => (
              <li key={to}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                      isActive
                        ? 'border border-blue-500/40 bg-blue-600/25 text-blue-200'
        ...
        <div className="flex items-center gap-2">
          <div className="hidden items-center gap-1.5 rounded-md border border-gray-800 bg-gray-900/60 px-3 py-1.5 font-mono text-xs text-gray-300 xl:flex">
            <Clock className="h-3.5 w-3.5 text-blue-400" aria-hidden />
            <span>{time}</span>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md border border-amber-500/40 bg-amber-600/20 px-3 py-1.5 text-xs font-medium text-amber-300 transition-all hover:bg-amber-600/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 active:scale-95"
          ...
          {user && (
            <div className="flex items-center gap-2 border-l border-gray-800 pl-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-full border border-blue-500/50 bg-blue-600/30 text-blue-300">
              ...
              <button
                onClick={logout}
                className="rounded-md p-2 text-gray-400 transition-colors hover:bg-red-500/10 hover:text-red-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                title="Đăng xuất"
