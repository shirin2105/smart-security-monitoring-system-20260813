...
OUTPUT_DIR = WORK / 'phase5b_deimv2_tiling_sweep'
NORMALIZED_VAL_ANN = WORK / 'visdrone_phase5b_val_contiguous.json'
EVAL_CONFIG = WORK / 'visdrone_phase5b_eval.yml'
SUMMARY_JSON = OUTPUT_DIR / 'phase5b_summary.json'
SUMMARY_CSV = OUTPUT_DIR / 'phase5b_summary.csv'
SUMMARY_MD = OUTPUT_DIR / 'phase5b_summary.md'

EXPERIMENTS = [
    # Re-run the baseline and Phase-5 known points in the SAME session
    # so all timing numbers are directly comparable.
    {
        'name': 'baseline_640',
        'mode': 'baseline',
        'tile_size': None,
    ...
    {
        'name': 'tile640_overlap00',
        'mode': 'tile',
        'tile_size': 640,
        'overlap': 0.00,
    },
    {
        'name': 'tile640_overlap10',
        'mode': 'tile',
        'tile_size': 640,
        'overlap': 0.10,
    },
    {
        'name': 'tile640_overlap15',
        'mode': 'tile',
        'tile_size': 640,
        'overlap': 0.15,
    },
    ...
    # Main Phase-5B tile-size sweep.
    # The detector still receives 640x640; these are source-image crop sizes.
    {
        'name': 'tile768_overlap20',
        'mode': 'tile',
        'tile_size': 768,
        'overlap': 0.20,
    },
    {
        'name': 'tile896_overlap20',
        'mode': 'tile',
        'tile_size': 896,
        'overlap': 0.20,
    },
    {
        'name': 'tile960_overlap20',
        'mode': 'tile',
        'tile_size': 960,
        'overlap': 0.20,
    },
...
def install_dependencies():
    ...
    #
    # IMPORTANT:
    # Importing engine.core also imports DEIMv2's dataset package, which
    # imports faster_coco_eval even though Phase 5 is inference-only.
    run([
        sys.executable,
        '-m',
        'pip',
...
def verify_runtime_dependencies():
    ...
    for module_name, package_name in checks:
        try:
            ...
            print(
                f'[DEP OK] {module_name} '
                f'(package={package_name}, version={version})',
                flush=True,
            )
        except Exception as exc:
            failures.append(
                (module_name, package_name, repr(exc))
            )

    if failures:
        detail = '\n'.join(
            f'  {module} / {package}: {error}'
            for module, package, error in failures
        )
        raise RuntimeError(
            'Required Phase-5 dependencies are still missing '
            'after pip install:\n' + detail
        )
...
def merge_tile_detections(
    ...
    # Avoid in-place writes on tensors that may originate from
    # torch.inference_mode().
    boxes = boxes.clone()

    boxes = torch.stack(
        [
            boxes[:, 0].clamp(0, image_width),
            boxes[:, 1].clamp(0, image_height),
            boxes[:, 2].clamp(0, image_width),
            boxes[:, 3].clamp(0, image_height),
        ],
        dim=1,
    )
    ...
    for index, image_info in enumerate(images, start=1):
        ...
        if mode == 'baseline':
            ...
            for x0, y0, x1, y1 in tiles:
                ...
                #
                # PyTorch >= 2.10: infer_pil() runs under
                # @torch.inference_mode(), so returned tensors are
                # "inference tensors". They cannot be modified in-place
                ...
                #   tile_boxes[:, [1, 3]] += y0
                #
                # Clone first and shift out-of-place instead.
                tile_boxes = (
                    tile_boxes.clone()
                    + torch.tensor(
                        [
                        ...
                        dtype=tile_boxes.dtype,
                    )
                )

                all_labels.append(tile_labels.clone())
                all_boxes.append(tile_boxes)
                all_scores.append(tile_scores.clone())
...
def save_summary(results):
    ...
    lines = [
        '# DEIMv2 Phase 5B — Tiling accuracy/latency sweep',
        '',
...
def print_final_summary(results):
    ...
    print('\n' + '=' * 96, flush=True)
    print('PHASE 5B FINAL COMPARISON', flush=True)
    print('=' * 96, flush=True)
    ...
    # Phase-5B target: keep most of the small-object gain while recovering speed.
    feasible = [
        item for item in tiled_results
        if item['AP_small'] >= 0.17
        and item['AR_small'] >= 0.33
    ]

    best_tradeoff = (
        max(
            feasible,
            key=lambda item: item['fps_original_images'],
        )
        if feasible
        else None
    )
    ...
    print(
        ...
        f"APs={best_accuracy['AP_small']:.4f} | "
        f"ARs={best_accuracy['AR_small']:.4f} | "
        f"FPS={best_accuracy['fps_original_images']:.2f}",
        flush=True,
    )

    if best_tradeoff is not None:
        print(
            '[BEST TRADE-OFF under APs>=0.17 and ARs>=0.33] '
            f"{best_tradeoff['name']} | "
            f"APs={best_tradeoff['AP_small']:.4f} | "
            f"ARs={best_tradeoff['AR_small']:.4f} | "
            f"FPS={best_tradeoff['fps_original_images']:.2f}",
            flush=True,
        )
    else:
        print(
            '[BEST TRADE-OFF] No tiled configuration met '
            'AP-small>=0.17 and AR-small>=0.33.',
            flush=True,
        )
    print(
        'Phase-5B target: AP-small >= 0.17, AR-small >= 0.33, maximize FPS; ideal FPS >= 8-12.',
        flush=True,
    )
    print('\nOutputs:', flush=True)
